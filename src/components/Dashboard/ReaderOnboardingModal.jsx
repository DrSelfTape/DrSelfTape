...
export default function ReaderOnboardingModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[998] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div
        className="bg-[#1A1A2E] border border-[#FF8280]/30 rounded-2xl px-6 py-8 flex flex-col items-center gap-5 shadow-2xl max-w-sm w-full"
        style={{ animation: 'badgePop 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards' }}
      >
        {/* Icons cluster */}
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-[#FF8280]/15 flex items-center justify-center">
            <Users2 className="w-6 h-6 text-[#FF8280]" />
          </div>
          ...
          <div className="w-12 h-12 rounded-full bg-[#FF8280]/15 flex items-center justify-center -ml-2">
            <HeartHandshake className="w-6 h-6 text-[#FF8280]" />
          </div>
        </div>
