...
export const useScriptAudioPlayer = ({
  ...
  const playLine = useCallback(async (lineIndex, options = {}) => {
    ...
    // IMPORTANT: Only cleanup if it's a different audio element
    // DON'T clear src here - let it be garbage collected naturally to avoid race conditions
    if (existingAudio && existingAudio !== activeAudioRef.current) {
