...
export function StudioRehearsalControls({ status, paused, actorTurn, readerMode, transcript, onNext, onPause, onResume, onEnd, onTimed }) {
  const heading = paused ? 'Scene paused' : actorTurn ? 'Your turn' : status === 'playing' ? 'Your scene partner' : status === 'error' ? 'Let’s try again' : 'Preparing your next line';
  const detail = paused ? 'Take a moment. Resume when you’re ready.' : actorTurn ? readerMode === 'listen' ? 'Listening to your line…' : 'Say your line, then tap Next line.' : status === 'playing' ? 'Listen. Let their words land.' : 'Stay in the scene.';
  return <footer className="studio-rehearsal-controls">
    <div className="studio-listening" role="status"><span className="studio-mic"><Mic size={23} strokeWidth={1.5} /></span><div><h2><span className="studio-status-dot" data-active={!paused && actorTurn} />{heading}</h2><p>{detail}</p></div></div>
    {transcript && actorTurn && <p className="studio-transcript">“{transcript}”</p>}
    <button type="button" className="studio-primary" onClick={paused ? onResume : onNext} disabled={!paused && !actorTurn}>
      {paused ? 'Resume scene' : 'Next line'}<ArrowRight size={20} strokeWidth={1.5} aria-hidden="true" />
    </button>
    {readerMode === 'listen' && <button type="button" className="studio-text-button" onClick={onTimed}>Switch to timed mode</button>}
