...
const TYPE_COLORS = {
  film: 'var(--aurora-rose)',
  commercial: 'var(--aurora-sky)',
  theatrical: 'var(--aurora-purple)',
  industrial: 'var(--aurora-dim)',
  theater: 'var(--aurora-mint)',
  voiceover: 'var(--aurora-heritage-gold)',
};
...
const LoadingSkeleton = ({ className = 'h-28' }) => (
  <div className={`animate-pulse rounded-xl ${className}`} style={{ background: 'var(--aurora-glass-strong)' }} />
);
...
export default function Reports() {
  ...
  const typeData = Object.entries(r.type_breakdown || {}).map(([key, count]) => ({
    ...
    value: count,
    color: TYPE_COLORS[key] || 'var(--aurora-rose)',
  }));

  if (!loading && error) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold" style={{ color: 'var(--aurora-text)', fontFamily: '"Space Grotesk", sans-serif' }}>Career Reports</h1>
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: 'rgba(255,179,193,0.18)' }}>
            <svg className="w-8 h-8" style={{ color: 'var(--aurora-accent-deep)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
            </svg>
          </div>
          <h2 className="text-xl font-semibold mb-2" style={{ color: 'var(--aurora-text)' }}>Could not load reports</h2>
          <p className="max-w-md mb-6" style={{ color: 'var(--aurora-sub)' }}>Please try again.</p>
          <button
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="text-xl font-semibold mb-2" style={{ color: 'var(--aurora-text)' }}>No data yet</h2>
          <p className="max-w-md" style={{ color: 'var(--aurora-sub)' }}>
        ...
        <div className="flex gap-2">
          {RANGE_OPTIONS.map((opt) => (
            <button
              ...
              style={{
                background: range === opt.value ? 'var(--aurora-heritage-gold)' : 'var(--aurora-glass-strong)',
                ...
                <AreaChart data={r.auditions_by_month || []}>
                  <defs>
                    <linearGradient id="colorAuditions" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="var(--aurora-rose)" stopOpacity={0.3} />
                      <stop offset="95%" stopColor="var(--aurora-rose)" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--aurora-line)" />
                  <XAxis dataKey="month" tick={{ fontSize: 12, fill: 'var(--aurora-dim)' }} axisLine={false} tickLine={false} />
                  <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: 'var(--aurora-dim)' }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid var(--aurora-line)', backgroundColor: 'var(--aurora-surface-solid)', color: 'var(--aurora-text)' }} />
                  <Area type="monotone" dataKey="count" stroke="var(--aurora-rose)" strokeWidth={2} fill="url(#colorAuditions)" name="Auditions" />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
                ...
                <AreaChart data={r.submissions_by_month || []}>
                  <defs>
                    <linearGradient id="colorSubs" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="var(--aurora-sky)" stopOpacity={0.3} />
                      <stop offset="95%" stopColor="var(--aurora-sky)" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--aurora-line)" />
                  <XAxis dataKey="month" tick={{ fontSize: 12, fill: 'var(--aurora-dim)' }} axisLine={false} tickLine={false} />
                  <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: 'var(--aurora-dim)' }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid var(--aurora-line)', backgroundColor: 'var(--aurora-surface-solid)', color: 'var(--aurora-text)' }} />
                  <Area type="monotone" dataKey="count" stroke="var(--aurora-sky)" strokeWidth={2} fill="url(#colorSubs)" name="Submissions" />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={funnelData} layout="vertical" barSize={28}>
                  ...
                  <XAxis type="number" allowDecimals={false} tick={{ fontSize: 12, fill: 'var(--aurora-dim)' }} axisLine={false} tickLine={false} />
                  ...
                  <Tooltip formatter={(value) => [value, 'Auditions']} contentStyle={{ borderRadius: '8px', border: '1px solid var(--aurora-line)', backgroundColor: 'var(--aurora-surface-solid)', color: 'var(--aurora-text)' }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                  ...
                  {r.top_casting_offices.map((office, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 rounded-full h-3 overflow-hidden" style={{ background: 'var(--aurora-glass-strong)' }}>
                          <div
                            className="h-full rounded-full transition-all"
                            style={{ width: `${(office.count / maxCount) * 100}%`, background: 'var(--aurora-heritage-gold)' }}
                          />
                        </div>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col items-center justify-center py-8">
              {r.busiest_month ? (
                <>
                  <div className="text-4xl font-bold aurora-mono mb-2" style={{ color: 'var(--aurora-accent-deep)' }}>{r.busiest_month}</div>
