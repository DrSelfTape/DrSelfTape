...
export default function AuditionGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGenerated(true);
      // Track + tutorial
      try { const { markStep } = await import('../../../components/Dashboard/TutorialChecklist'); markStep('generate_scene'); } catch { /* Optional operation failed; continue with the existing fallback. */ }
      try { const { trackEvent, Events } = await import('../../../utils/analytics'); trackEvent(Events.GENERATE_SCENE, { genre, tone, difficulty }); } catch { /* Optional operation failed; continue with the existing fallback. */ }
    } catch (err) {
  ...
  const handleExportPDF = async () => {
    // window.open('', '_blank') returns null in WKWebView (iOS), so the old
    // win.document.write threw and the export silently failed. Build the HTML
